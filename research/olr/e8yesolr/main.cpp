...
const OrenNayarBRDF leftWall(Vec(.75,.25,.25),0.8),
rightWall(Vec(.25,.25,.75),0.8),
otherWall(Vec(.75,.75,.75),0.8),
blackSurf(Vec(0.0,0.0,0.0),0.8),
...
void aces_tonemap(std::vector<Vec>& c, double exposure)
{
        ...
        for (unsigned i = 0; i < c.size(); i ++) {
                ...
                c[i] = color.mult(color * A + Vec(B, B, B)).div(color.mult(color * C + Vec(D, D, D)) + Vec(E, E, E));
        }
}
