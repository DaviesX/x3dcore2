...
std::vector<e8util::vec3> e8::normal_pathtracer::sample(e8util::rng &,
                                                        std::vector<e8util::ray> const &rays,
                                                        if_path_space const &path_space,
                                                        if_light_sources const & /* light_sources */,
                                                        unsigned) const
...
e8::if_light const *e8::direct_pathtracer::sample_illum_source(
    e8util::rng &rng,
    e8util::vec3 &p,
    e8util::vec3 &n,
    float &density,
                                                        ...
                                                        unsigned multi_light_samps) const
{
    ...
    for (unsigned k = 0; k < multi_light_samps; k++) {
        ...
        rad += transport_illum_source(*light, p, n, target_vert, target_o_ray, path_space)
               / density;
    }
...
unsigned e8::unidirect_pathtracer::sample_path(e8util::rng &rng,
                                               ...
                                               unsigned max_depth) const
{
    if (depth == max_depth)
        return depth;
    float w_dens;
    e8util::vec3 const &i = sampled_path[depth - 1]
                                .vert.mat->sample(rng,
                                                  sampled_path[depth - 1].vert.normal,
                                                  -sampled_path[depth - 1].o,
                                                  w_dens);
    e8::intersect_info const &next_vert = path_space.intersect(
        e8util::ray(sampled_path[depth - 1].vert.vertex, i));
    if (next_vert.valid) {
...
e8util::vec3 e8::unidirect_pathtracer::transport_subpath(e8util::vec3 const &src_rad,
                                                         ...
                                                         bool is_forward) const
{
    ...
    if (is_forward) {
        ...
        return transport
               * sampled_path[sub_path_len - 1]
                     .vert.mat->eval(sampled_path[sub_path_len - 1].vert.normal,
                                     appending_ray,
...
float e8::unidirect_pathtracer::subpath_density(float src_dens,
                                                sampled_pathlet const *sampled_path,
                                                unsigned path_start,
...
e8util::vec3 e8::unidirect_pathtracer::sample_indirect_illum(e8util::rng &rng,
                                                             e8util::vec3 const &o,
                                                             ...
                                                             unsigned multi_indirect_samps) const
{
    ...
    for (unsigned k = 0; k < multi_indirect_samps; k++) {
        ...
        if (indirect_vert.valid) {
            e8util::vec3 const &indirect = sample_indirect_illum(rng,
                                                                 -i,
                                                                 indirect_vert,
                                                                 path_space,
                                                                 light_sources,
...
e8util::vec3 e8::bidirect_lt2_pathtracer::join_with_light_paths(e8util::rng &rng,
                                                                e8util::vec3 const &o,
                                                                ...
                                                                unsigned cam_path_len) const
{
    ...
    if (cos_wo > 0.0f && cos_wi > 0.0f && cos_w2 > 0.0f
        ...
        e8util::vec3 f2 = light_illum * terminate.mat->eval(terminate.normal, join_path, tray)
                          * cos_w2;
        p2_direct = f2 * cos_wo / (distance * distance) * poi.mat->eval(poi.normal, o, -join_path)
                    * cos_wi;
        if (cam_path_len == 0)
...
e8util::vec3 e8::bidirect_lt2_pathtracer::sample_indirect_illum(e8util::rng &rng,
                                                                e8util::vec3 const &o,
                                                                e8::intersect_info const &vert,
                                                                if_path_space const &path_space,
                                                                if_light_sources const &light_sources,
...
e8::if_light const *e8::bidirect_mis_pathtracer::sample_illum_source(
    e8util::rng &rng,
    e8util::vec3 &p,
    e8util::vec3 &n,
    e8util::vec3 &w,
...
e8util::vec3 e8::bidirect_mis_pathtracer::sample_all_subpaths(sampled_pathlet const *cam_path,
    ...
    for (unsigned i = 1; i <= cam_path_len; i++) {
        for (unsigned j = 0; j <= light_path_len; j++) {
            ...
            if (j == 0) {
                ...
                e8util::vec3 join_path = cam_path[i - 1].vert.vertex
                                         - light_path[j - 1].vert.vertex;
                float distance = join_path.norm();
                join_path = join_path / distance;
                ...
                if (cos_wo > 0.0f && cos_wi > 0.0f && cos_w2 > 0.0f
                    ...
                    e8util::vec3 transported_light_illum = light_subpath_rad * cos_wo
                                                           / (distance * distance);
...
e8util::vec3 e8::bidirect_mis_pathtracer::sample_all_subpaths_opti(
    sampled_pathlet const *cam_path,
    unsigned max_cam_path_len,
    sampled_pathlet const *light_path,
    unsigned max_light_path_len,
    e8util::vec3 const &light_p,
    e8util::vec3 const &light_n,
    float pdf_light_p_dens,
    if_light const &light,
        ...
        while (light_plen < plen && light_plen <= max_light_path_len) {
            ...
            if (light_plen == 0 && cam_plen == 0) {
                ...
                if (cam_path[0].vert.light != nullptr) {
                    path_rad = cam_path[0].vert.light->emission(-cam_path[0].o,
                ...
                e8util::vec3 transported_light_illum
                    = transport_illum_source(light,
                                             light_p,
                                             light_n,
                                             cam_path[cam_plen - 1].vert,
                                             -cam_path[cam_plen - 1].o,
                                             path_space)
                      / dens;
                ...
                e8util::vec3 join_path = cam_path[cam_plen - 1].vert.vertex
                                         - light_path[light_plen - 1].vert.vertex;
                float distance = join_path.norm();
                join_path = join_path / distance;
                e8util::ray join_ray(cam_path[cam_plen - 1].vert.vertex, join_path);
                float cos_w2 = light_path[light_plen].vert.normal.inner(
                    -light_path[light_plen - 1].o);
                                                              ...
                                                              unsigned) const
{
    ...
    for (unsigned i = 0; i < rays.size(); i++) {
        ...
        rad[i] = sample_all_subpaths_opti(cam_path,
                                          cam_path_len,
                                          light_path,
                                          light_path_len,
                                          light_p,
