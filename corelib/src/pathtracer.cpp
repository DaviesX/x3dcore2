...
e8::position_pathtracer::sample(e8util::rng&,
                                           ...
                                           unsigned multi_light_samps) const
{
        ...
        for (unsigned k = 0; k < multi_light_samps; k ++) {
                e8util::vec3 p, n;
                float density;
                e8::if_light const* light = sample_illum_source(rng, p, n, density, target_vert, light_sources);
                              ...
                              unsigned multi_light_samps) const
{
        ...
        for (unsigned i = 0; i < rays.size(); i ++) {
                ...
                if (vert.valid) {
                        // compute radiance.
                        rad[i] = sample_direct_illum(rng, -ray.v(), vert, path_space, light_sources, multi_light_samps);
                        if (vert.light)
                                                ...
                                                unsigned multi_indirect_samps) const
{
        ...
        // direct.
        e8util::vec3 const& direct = sample_direct_illum(rng, o, vert, path_space, light_sources, multi_light_samps);
...
std::vector<e8util::vec3>
e8::unidirect_pathtracer::sample(e8util::rng& rng,
                                 std::vector<e8util::ray> const& rays,
                                 if_path_space const& path_space,
                                 if_light_sources const& light_sources,
                                 unsigned /* n */) const
{
        ...
        for (unsigned i = 0; i < rays.size(); i ++) {
                ...
                if (vert.valid) {
                        // compute radiance.
                        e8util::vec3 const& p2_inf = sample_indirect_illum(rng, -ray.v(), vert, path_space, light_sources, 0, 1, 1);
                        if (vert.light)
                                                   ...
                                                   unsigned depth) const
{
        ...
        e8util::vec3 const& bidirect = join_with_light_paths(rng, o, vert, path_space, light_sources, depth);
        ...
        if (indirect_info.valid) {
                e8util::vec3 const& indirect = sample_indirect_illum(rng, -i, indirect_info,
                                                                     path_space, light_sources, depth + 1);
                e8util::vec3 const& brdf = vert.mat->eval(vert.normal, o, i);
                float cos_w = vert.normal.inner(i);
                if (cos_w < 0.0f)
                        return 0.0f;
                                    ...
                                    unsigned) const
{
        ...
        for (unsigned i = 0; i < rays.size(); i ++) {
                ...
                if (vert.valid) {
                        // compute radiance.
                        e8util::vec3 const& p2_inf = sample_indirect_illum(rng, -ray.v(), vert, path_space, light_sources, 0);
                        if (vert.light)
                                rad[i] = p2_inf + vert.light->emission(-ray.v(), vert.normal);
                        else
                                rad[i] = p2_inf;
...
e8util::vec3
e8::bidirect_mis_pathtracer::sample_all_subpaths_opti(sampled_pathlet const* cam_path,
        ...
        for (unsigned plen = 1; plen <= max_cam_path_len + max_light_path_len + 1; plen ++) {
                unsigned cam_plen = plen - 1;
                unsigned light_plen = 0;

                while (light_plen < plen) {
                        if (cam_plen <= max_cam_path_len &&
                            light_plen <= max_light_path_len) {
                                ...
                                if (light_plen == 0 && cam_plen == 0) {
                                        ...
                                        if (cam_path[0].vert.light != nullptr) {
                                                path_rad = cam_path[0].vert.light->emission(-cam_path[0].o, cam_path[0].vert.normal);
                                        }
                                        ...
                                        e8util::vec3 join_path = cam_path[cam_plen - 1].vert.vertex - light_path[light_plen - 1].vert.vertex;
                                        float distance = join_path.norm();
                                        join_path = join_path/distance;
                                        e8util::ray join_ray(cam_path[cam_plen - 1].vert.vertex, join_path);
                                        float cos_w2 = light_path[light_plen].vert.normal.inner(-light_path[light_plen - 1].o);
        ...
        return paritition_rad_sum/(max_cam_path_len + max_light_path_len + 1);
}
