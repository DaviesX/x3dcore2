...
e8util::flex_config e8::if_render_pipeline::config() const
{
    return m_old_config;
}

e8::objdb &e8::if_render_pipeline::objdb()
{
    return m_objdb;
}

bool e8::if_render_pipeline::is_running() const
{
    return m_is_running;
}

void e8::if_render_pipeline::enable()
{
    m_is_running = true;
}

void e8::if_render_pipeline::disable()
{
    m_is_running = false;
}

unsigned e8::if_render_pipeline::frame_no() const
{
    return m_frame_no;
}
...
e8util::flex_config e8::pt_render_pipeline::config_protocol() const
{
    ...
    config.enum_vals["path_tracer"] = std::set<std::string>{"normal",
                                                            "position",
                                                            "direct",
                                                            "unidirectional",
                                                            "bidirectional_lt2",
...
{
    // update.
    diff.find_enum("path_tracer",
                   [this](std::string const &tracer_type, e8util::flex_config const * /*config*/) {
                       delete m_renderer;
                       e8::pathtracer_factory::pt_type pt_type
                           = e8::pathtracer_factory::pt_type::normal;
    ...
    diff.find_enum("path_space",
                   [this](std::string const &path_space_type,
    ...
    diff.find_enum("light_sources",
                   [this](std::string const &light_sources_type,
                          e8util::flex_config const * /*config*/) {
