...
template<unsigned N, typename T>
vec<N, T> vec<N, T>::operator+(vec const &rhs) const
{
    vec<N, T> v;
    for (unsigned i = 0; i < N; i++)
        v(i) = (*this)(i) + rhs(i);
...
template<unsigned N, typename T>
T vec<N, T>::operator()(unsigned i) const
{
    return e[i];
}

template<unsigned N, typename T>
bool vec<N, T>::operator==(vec const &rhs) const
{
    for (unsigned i = 0; i < N; i++) {
...
template<unsigned N, typename T>
vec<N, T> vec<N, T>::operator*(T k) const
{
    vec<N, T> v;
    for (unsigned i = 0; i < N; i++)
        v(i) = k * (*this)(i);
...
template<unsigned N, typename T>
inline vec<N, T> operator*(T k, vec<N, T> const &rhs)
{
    return rhs * k;
}

template<unsigned N, typename T>
T vec<N, T>::inner(vec const &rhs) const
{
    T v = 0;
    for (unsigned i = 0; i < N; i++)
        v += (*this)(i) *rhs(i);
...
template<unsigned N, typename T>
vec<N, T> &vec<N, T>::operator+=(vec const &rhs)
{
    for (unsigned i = 0; i < N; i++)
        (*this)(i) += rhs(i);
    return *this;
...
template<unsigned N, typename T>
vec<N, T> vec<N, T>::outer(vec const &rhs) const
{
    if (N != 3)
...
template<unsigned N, typename T>
vec<N, T> vec<N, T>::normalize() const
{
    return 1.0f / norm() * (*this);
}

template<unsigned N, typename T>
vec<N + 1, T> vec<N, T>::homo(T p) const
{
    T v[N + 1];
    for (unsigned i = 0; i < N; i++)
        v[i] = (*this)(i);
...
template<unsigned N, typename T>
vec<N, T> vec<N, T>::at_least(T const &val) const
{
    T v[N];
    for (unsigned i = 0; i < N; i++)
...
template<unsigned N, typename T>
vec<N, T> vec<N, T>::at_most(T const &val) const
{
    T v[N];
    for (unsigned i = 0; i < N; i++)
        v[i] = val < (*this)(i) ? val : (*this)(i);
...
template<unsigned M, unsigned N, typename T>
T &mat<M, N, T>::operator()(unsigned i, unsigned j)
{
    return e[j][i];
}
...
template<unsigned M, unsigned N, typename T>
mat<M, N, T> mat<M, N, T>::operator*(T k) const
{
    mat<M, N, T> m;
    for (unsigned j = 0; j < N; j++) {
        for (unsigned i = 0; i < M; i++) {
...
T mat<M, N, T>::det(unsigned next_row[M + 1], unsigned curr_col, unsigned removed_col) const
{
    ...
    for (unsigned i = 0, prev_row = 0; curr_row != M + 1;
        ...
        T m_ij = det(next_row,
                     (curr_col + 1 == removed_col) ? (curr_col + 2) : (curr_col + 1),
                     removed_col);
...
template<unsigned M, unsigned N, typename T>
T mat<M, N, T>::det(mat const &adj) const
{
    T d = 0;
    for (unsigned i = 0; i < M; i++) {
        d += (*this)(i, 0) * adj(0, i);
...
// Transposed comatrix.
template<unsigned M, unsigned N, typename T>
mat<M, N, T> mat<M, N, T>::adjugate() const
{
...
template<unsigned M, unsigned N, typename T>
T mat<M, N, T>::det() const
{
    return det(adjugate());
}

template<unsigned M, unsigned N, typename T>
mat<M, N, T> mat<M, N, T>::operator^(int e) const
{
    if (M != N)
...
template<unsigned M, unsigned N, typename T>
bool mat<M, N, T>::operator==(mat const &rhs) const
{
    for (unsigned j = 0; j < N; j++) {
        for (unsigned i = 0; i < M; i++) {
...
inline bool ray::intersect(vec3 const &v0,
                           vec3 const &v1,
                           vec3 const &v2,
                           float t_min,
                           float t_max,
...
inline vec3 ray::o() const
{
    return m_o;
}

inline vec3 ray::v() const
{
    return m_v;
}

inline vec3 ray::v_inv() const
{
    return m_v_inv;
}
...
inline bool aabb::is_empty() const
{
    return m_is_empty;
}
...
inline float aabb::enclosing_radius() const
{
    return (m_max - m_min).norm() / 2;
}

inline vec3 aabb::centroid() const
{
...
inline mat44 mat44_lookat(vec3 const &eye, vec3 const &center, vec3 const &up)
{
    ...
    mat44 const &l = mat44(
        {v(0), u(0), -w(0), 0, v(1), u(1), -w(1), 0, v(2), u(2), -w(2), 0, 0, 0, 0, 1});
...
inline mat44 mat44_normal(mat44 const &affine)
{
    return ~(affine ^ (-1));
}
