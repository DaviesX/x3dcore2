...
{
        void operator()(std::string const& /* enum_sel */,
                        flex_config const* /* enum_config */) {}
};
...
template<typename ReadOp>
void
flex_config::find_bool(std::string const& key, ReadOp read_op) const
{
...
template<typename ReadOp>
void
flex_config::find_int(std::string const& key, ReadOp read_op) const
{
...
template<typename ReadOp>
void
flex_config::find_float(std::string const& key, ReadOp read_op) const
{
...
template<typename ReadOp>
void
flex_config::find_str(std::string const& key, ReadOp read_op) const
{
...
template<typename ReadOpEnum>
void
flex_config::find_enum(std::string const& key, ReadOpEnum read_op) const
{
        auto it = enum_sel.find(key);
        if (it != enum_sel.end()) {
                auto config_it = enum_val_configs.find(key + "." + it->second);
                read_op(it->second,
                        config_it != enum_val_configs.end() ?
                                        &config_it->second :
                                        nullptr);
        }
}
